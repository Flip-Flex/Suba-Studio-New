...
for key, data in updates.items():
    pattern = r'(\"' + key + r'\": \{[\s\S]*?tagline: )\".*?\"([\s\S]*?description: )\".*?\"([\s\S]*?whyChooseTitle: )\".*?\"([\s\S]*?whyChooseText: )\".*?\"'
    replacement = r'\g<1>\"' + data['tagline'] + r'\"\g<2>\"' + data['description'] + r'\"\g<3>\"' + data['whyChooseTitle'] + r'\"\g<4>\"' + data['whyChooseText'] + r'\"'
    content = re.sub(pattern, replacement, content)

with open('src/data/categoryContent.ts', 'w', encoding='utf-8') as f:
    f.write(content)

print('Updated quotes successfully!')
